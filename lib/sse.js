...
SSE.prototype.handler = function(req, res) {
  var oplogConnectionString = this.options.sseConnectionString || '';
  const connectionStrings = oplogConnectionString.split(',');
  ...
  connetionsStrings.forEach((connection) => {
    const db = mongoose.createConnection(connection, gooseOpt);
        ...
        .then(function(query) {
          ...
          var consume = hl().consume(function(err, chunk, push, next) {
            ...
            var resourceNames = _.map(that.routeNames, function(routeName) {
              var pluralName = that.options.inflect
                ? inflect.pluralize(routeName)
                : routeName;
