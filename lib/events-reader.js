...
module.exports = function(harvesterApp) {
  return function(oplogMongodbUri) {
    ...
    EventsReader.prototype.processDocHandlers = function(doc) {
      ...
      if (doc != null) {
        debug(
          'got data from oplog ' + JSON.stringify(doc) + ' ts: ' + logTs(doc.ts)
        );

        var matchedChangeHandlers = matchChangeChandlers(
          harvesterApp.changeHandlers,
          doc.ns
        );
    ...
    var processWithHandlerT = throttle(processWithHandler, {
      ...
      window: 1,
      limit: parseInt(
        _.get(harvesterApp, 'options.eventsReaderThrottleLimit'),
        10
      ) || 100,
    });
    ...
    function checkFilterExecuteHandler(
      that,
      id,
      dfd,
      opFn,
    ...
    EventsReader.prototype.updateCheckpointAndReschedule = function(doc) {
      ...
      if (doc != null) {
        ...
        if (!matchCheckpoint) {
          ...
          checkpointWriter.checkpointEventEmitter.emit(
            'newCheckpoint',
            that.checkpoint.id,
            doc
          );
