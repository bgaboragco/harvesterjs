...
module.exports = function(adapter, schemas) {
  function linked(body, inclusions) {
    ...
    function extractPrimaryResource(body) {
      return _.head(
        _.filter(_.keys(body), function(key) {
    ...
    function isRemote(branch, key) {
      return (
        branch != null &&
        branch.refs != null &&
        branch.refs[key] != null &&
        branch.refs[key].def != null &&
        branch.refs[key].def.baseUri != null
      );
    }
    ...
    function buildInclusionBranch(inclusionTokens, schema) {
      var inclusionToken = _.head(inclusionTokens);
      var type = _.isArray(schema[inclusionToken])
        ? schema[inclusionToken][0]
        : schema[inclusionToken];
      ...
      if (baseUri) {
        var remoteIncludes = _.drop(
          inclusionTokens,
          _.indexOf(inclusionTokens, inclusionToken) + 1
        ).join('.');
        ...
        if (tokensRemaining.length == 0) {
          ...
          return _.merge(
            linkDescriptor,
            buildInclusionBranch(tokensRemaining, schemas[normalisedType])
          );
        }
      }
    ...
    return fetchLinked({}, resources, inclusionTree).then(function(linked) {
  ...
  function fetchLinked(fetchedIds, resources, inclusionBranch) {
    return Promise.all(
      _.map(_.keys(inclusionBranch ? inclusionBranch.refs : []), function(
        inclusionRefKey
      ) {
        return fetchResources(
          ...
          if (result) {
            ...
            var key;
            var mergedLinkedResources = _.reduce(
              result,
              function(acc, linkedResources, linkedResName) {
                key = linkedResName;
                var concatExisting = _.merge(
                  linkedResources,
                  acc.linked[linkedResName]
                );
            ...
            return fetchLinked(
              fetchedIds,
              mergedLinkedResources.linked[key],
              inclusionBranch.refs[inclusionRefKey]
            ).then(function(result) {
  ...
  function fetchResources(
    ...
    if (linkedIds && linkedIds.length > 0) {
      ...
      if (!inclusionDescriptor.def.baseUri) {
        return (
          adapter
            .findMany(type, remainingIds, remainingIds.length)
        ...
        return $http(
          inclusionDescriptor.def.baseUri +
            '/' +
            resourceName +
            '?id=' +
            remainingIds.join(',') +
