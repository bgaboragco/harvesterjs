...
function createRouteInterceptor(routeMethod) {
  return function routeInterceptor(request, response) {
    var authorizationStrategy =
      routeMethod.options.harvester.authorizationStrategy;
    var thatRouteInterceptorContext = this;
    // noinspection UnnecessaryLocalVariableJS
    var thatRouteInterceptorArguments = arguments;

    function callRouteHandler() {
      routeMethod.handlerFunc.apply(
        thatRouteInterceptorContext,
        thatRouteInterceptorArguments
      );
    }
    ...
    function isAuthorizationRequired() {
      ...
      var authorizationStrategyPresent =
        authorizationStrategy instanceof Function;
      return (
        authorizationEnabled &&
        !methodNotAllowed &&
        authorizationStrategyPresent
      );
    }
    ...
    function safelyCallRouteHandler(authResult) {
      if (authResult instanceof Error) {
        return sendError(request, response, authResult);
      }
      try {
        callRouteHandler();
      } catch (error) {
        console.error(
          'This should never happen, but route handler for',
          routeMethod.options.method,
          routeMethod.options.route,
          'threw exception!!'
        );
    ...
    function handleAuthorizationResult(authorizationResult) {
      ...
      if (
        ...
        authorizationResult.then instanceof Function
      ) {
        authorizationResult
          .then(safelyCallRouteHandler)
          .catch(function(result) {
            var err = new JSONAPI_Error({ status: 403 });
            if (result instanceof JSONAPI_Error) {
              err = result;
            }
    ...
    if (isAuthorizationRequired()) {
      var rolesAllowed = _.isEmpty(routeMethod.roles)
        ? routeMethod.options.resource.roles
        : routeMethod.roles;
      var authorizationResult = authorizationStrategy(
        request,
        routeMethod.getPermissionName(),
        rolesAllowed || []
      );
...
function RouteMethod(options) {
  if (options.harvester == null) {
    throw new Error(
      'Options must include reference to harvester (property harvester)'
    );
  }
...
RouteMethod.prototype.register = function() {
  ...
  if (this.options.notAllowed) {
    ...
    this.options.harvester.router[method](
      this.options.route,
      createRouteInterceptor(thatRouteMethod)
    );
  }
  return this;
};
...
RouteMethod.prototype.getPermissionName = function() {
  return (
    this.options.resource.name +
    '.' +
    (this.options.permissionSuffix || this.options.method)
  );
};
