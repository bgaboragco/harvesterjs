...
function Adapter() {
  ...
  adapter._init = function(options) {
    ...
    const gooseOpt = _.cloneDeep(options.flags || {});

    if (gooseOpt.server || gooseOpt.replset) {
      console.error(`Invalid options: ${JSON.stringify(gooseOpt)}`);
      console.error('Old v4.x Mongoose options structure is no longer valid. Use the v5.x format instead. (https://mongoosejs.com/docs/connections.html#v5-changes)');
    }

    // Set bluebird as default promise handler
    mongoose.Promise = require('bluebird');
    gooseOpt.promiseLibrary = require('bluebird');

    // always include keepAlive in connection options
    // see: http://mongoosejs.com/docs/connections.html
    gooseOpt.keepAlive = true;
  ...
  adapter._serialize = function(model, resource) {
    ...
    if (
      resource.hasOwnProperty('links') &&
      typeof resource.links === 'object'
    ) {
  ...
  adapter._deserialize = function(model, resource) {
    ...
    model.schema.eachPath(function(path, type) {
      ...
      var hasManyRef =
        type.options.type && type.options.type[0] && type.options.type[0].ref;
      var isRef = !!(type.options.ref || hasManyRef);
      var instance =
        type.instance || (type.caster ? type.caster.instance : undefined);
      if (
        path != '_id' &&
        instance == 'String' &&
        uuidRegexp.test(resource[path]) &&
        isRef
      ) {
