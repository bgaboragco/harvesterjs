...
Harvester.prototype._init = function(options) {
  ...
  route.getRouteMethodNames().forEach(function(httpMethod) {
    harvesterInstance[httpMethod] = function() {
      var resource =
        harvesterInstance.createdResources[harvesterInstance._resource];
...
Harvester.prototype.resource = function(name, resourceSchema, options) {
  ...
  let mongooseSchema = _this.adapter.schema(
    name,
    processedLegacySchema,
    options
  );
  this.createdResources = this.createdResources || {};
  this.createdResources[name] = new route(
    _this,
    name,
    _this.adapter.model(name, mongooseSchema),
    _.clone(resourceSchema),
    options
  );
  ...
  _.each(schema, function(schemaItem, key) {
    if (schemaItem.isJoi) {
      ...
      if (schemaItem._type === 'object') {
        var subSchema = {};
        _.forEach(schemaItem._inner && schemaItem._inner.children, function(
          child
        ) {
...
Harvester.prototype.roles = function() {
  this.createdResources[this._resource].roles = Array.prototype.slice.call(
    arguments
  );
  return this;
};
