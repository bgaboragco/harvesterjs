...
describe('EventSource implementation for resource changes', function() {
  ...
  describe('Server Sent Events', function() {
    ...
    before(function() {
      ...
      return seeder(harvesterApp, baseUrl).dropCollections(
        'books',
        'dvds',
        'superHeros'
      );
    ...
    describe('When I post to the newly created resource', function() {
      it('Then I should receive a change event with data but not the one before it', function(
        done
      ) {
    ...
    describe('When I post resource with uppercased characters in name', function() {
      it('Then I should receive a change event', function(done) {
        var eventSource = ess(baseUrl + '/superHeros/changes/stream', {
          retry: false,
    ...
    describe('when I ask for events with ids greater than a certain id with filters enabled', function() {
      it('I should get only one event without setting a limit', function(done) {
        ...
        var eventSource = ess(
          baseUrl +
            '/books/changes/stream?title=filtered&author=Asimov&limit=100',
          {
            retry: false,
            headers: {
    ...
    describe(
      'Given a resource x with property y ' + '\nWhen the value of y changes',
      function() {
        it(
          'Then an SSE is broadcast with event set to x_update, ID set to the oplog timestamp' +
            'and data set to an instance of x that contains document id and new value for property y',
          function(done) {
            var payloads = [
              {
                books: [
            ...
            $http
              ...
              .spread(function(res) {
                ...
                var eventSource = ess(baseUrl + '/books/changes/stream', {
                  ...
                  var data = JSON.parse(response.data);
                  if (counter === 0) {
                    $http.put(baseUrl + '/books/' + documentId, {
                      json: payloads[1],
                    });
                  }
                  if (counter === 1) {
    ...
    describe('When I update collection document directly through mongodb adapter', function() {
      ...
      it('Then SSE should broadcast event with data containing properties that has been changed', function(
        done
      ) {
        var expected = {
          _id: documentId,
