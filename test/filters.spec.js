...
describe('filters', function() {
  ...
  it('should allow top-level resource filtering based on empty property', function(
    done
  ) {
    request(config.baseUrl)
      .get('/people?nickname=,')
  ...
  it('should allow top-level resource filtering based on empty property', function(
    ...
    request(config.baseUrl)
      .get('/people?nickname=Pocahontas,')
      .expect('Content-Type', /json/)
      .expect(200)
      .end(function(error, response) {
        should.not.exist(error);
        ...
        _.map(body.people, 'nickname')
          .sort()
          .should.eql(['Pocahontas', undefined, undefined]);
        done();
      });
  });
  ...
  it('should allow combining top-level resource filtering for collection routes based on string & numeric values', function(
    ...
    request(config.baseUrl)
      .get('/people?name=Dilbert&appearances=3457')
      .expect('Content-Type', /json/)
      .expect(200)
      .end(function(error, response) {
        should.not.exist(error);
  ...
  it.skip(
    'should allow resource sub-document filtering based on a numeric value',
    function(done) {
      request(config.baseUrl)
        .get('/people?links.pets=2')
  ...
  it.skip(
    'should be possible to filter related resources by ObjectId',
    function(done) {
      var cmd = [
        {
          op: 'replace',
  ...
  it.skip(
    'should support filtering by id for one-to-one relationships',
    function(done) {
  ...
  it.skip('should support $in query against external refs values', function(
    done
  ) {
    new Promise(function(resolve) {
      request(config.baseUrl)
  ...
  it.skip('should be able to run $in query against nested fields', function(
    done
  ) {
    request(config.baseUrl)
  ...
  it.skip('should support or query', function(done) {
    request(config.baseUrl)
      .get('/people?(name=Dilbert|name=Ratbert)&sort=name')
      .expect(200)
      .end(function(err, res) {
        should.not.exist(err);
        var body = JSON.parse(res.text);
  ...
  it('should support lt query', function(done) {
    request(config.baseUrl)
      .get('/people?appearances=lt=1935')
      .expect(200)
      .end(function(err, res) {
        should.not.exist(err);
        var body = JSON.parse(res.text);
  ...
  it('should support le query', function(done) {
    request(config.baseUrl)
      .get('/people?appearances=le=1934')
      .expect(200)
      .end(function(err, res) {
        should.not.exist(err);
        var body = JSON.parse(res.text);
  ...
  it('should support gt query', function(done) {
    request(config.baseUrl)
      .get('/people?appearances=gt=1935')
      .expect(200)
      .end(function(err, res) {
        should.not.exist(err);
        var body = JSON.parse(res.text);
  ...
  it('should support ge query', function(done) {
    request(config.baseUrl)
      .get('/people?appearances=ge=3457')
      .expect(200)
      .end(function(err, res) {
        should.not.exist(err);
        var body = JSON.parse(res.text);
  ...
  it('should convert a range query with lt= and gt= into a mongo query object', function(
    ...
    let testUrl =
      '/pets?limit=100&adopted=ge=2015-10-08T18:40:28.000Z&adopted=le=2015-10-16T21:40:28.000Z';
