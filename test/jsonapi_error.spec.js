...
describe('jsonapi error handling', function() {
  ...
  describe('raise a JSONAPI_Error error in foobar before callback', function() {
    it('should respond with a 400 and content-type set to application/vnd.api+json', function(
      done
    ) {
  ...
  describe('bulk insert 2 entities into /foobars resource, each before callback rejects with an JSONAPI_Error', function() {
    it('should respond with a 400, content-type set to application/vnd.api+json and the errors object carries the first error encountered', function(
      done
    ) {
      request(config.baseUrl)
        .post('/foobars')
  ...
  describe('raise random error in the express req/res chain', function() {
    it('should respond with a 500 error and content-type set to application/vnd.api+json', function(
      done
    ) {
      request(config.baseUrl)
        .get('/random-error')
  ...
  describe('raise JSONAPI_Error with 400 status code in the express the req/res chain', function() {
    it('should respond with a 400 error and content-type set to application/vnd.api+json', function(
      done
    ) {
      request(config.baseUrl)
