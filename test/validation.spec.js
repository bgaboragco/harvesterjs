...
describe('validation', function() {
  ...
  describe('validation query', function() {
    var schema = {
      query: { offset: Joi.number().required().description('offset') },
    };
  ...
  describe('validation headers', function() {
    ...
    describe('when validating an invalid resource', function() {
      it('should resolve with errors', function() {
        ...
        expect(headerDetails[0].message).to.equal(
  ...
  describe('validation api calls', function() {
    ...
    describe('when a resource is POSTed with a malformed payload which has a required attribute "appearances" missing', function() {
      it('should resolve with a 400 and a validationErrorDetails section stating "appearances" is required', function(
        done
      ) {
        var pet = {
            name: 'Spot',
        ...
        request(config.baseUrl)
          ...
          .expect(function(res) {
            ...
            expect(bodyDetails[0].message).to.equal(
              '"appearances" is required'
            );
          })
          .end(function(err) {
            if (err) {
    ...
    describe('when a resource is PUT with a malformed payload which has an unknown attribute "foo" defined', function() {
      it('should resolve with a 400 and a validationErrorDetails section stating "foo" is not allowed', function(
        done
      ) {
        var pet = {
            name: 'Spot',
    ...
    describe('when a resource is PUT with a malformed payload which has multiple primary resource collection entries', function() {
      it('should resolve with a 400 and a validationErrorDetails section stating "pets" must contain 1 items', function(
        done
      ) {
        var pets = { pets: [] };
        pets.pets.push({
    ...
    describe('when resource has Joi.object property', function() {
      ...
      it('should NOT allow persisting object with missing required object property', function(
        done
      ) {
        var object = {};
        request(config.baseUrl)
      ...
      it('should NOT allow persisting object with additional inner property not defined in schema', function(
        done
      ) {
        var object = {
          foo: {
